...
const FEATURES = [
    'Plays music from this device - a folder you choose in the browser, or the local Spotifie app - never uploaded',
    'Browses a published global catalogue alongside that local music',
    'With the local Spotifie app, keeps imported audio private to the account that imported it',
...
function llmsTxt() {
    ...
    return [
        ...
        '## Architecture',
        '',
        '- Runs as a web app, or from a small local server that also indexes and streams the music on that machine.',
        '- Music on the device is indexed locally. Those audio files are never uploaded.',
        '- The published catalogue is stored separately and is public to read.',
        '- With the local server, audio a person imports is private to that account and served only to it.',
        '- Supabase provides sign-in, profiles and the published catalogue; music on a person\'s device never goes there.',
        '',
        '## Local and global',
        '',
        ...
        '- Audio and artwork from a person\'s device are never uploaded to a third-party service.',
